...
def loras_oversampling(min_class_points, k, num_shadow_points, list_sigma_f, num_generated_points, num_hyp_points,
                       num_aff_comb, random_state):
    ...
    for neighbourhood in neighbourhoods:
        arg = (neighbourhood, k, num_shadow_points, list_sigma_f, num_generated_points, num_hyp_points, num_aff_comb,
               random_state)
        args.append(arg)
    # Generating points
    loras_set = []
    ...
    for arg in args:
        neighbourhood_loras_set = neighbourhood_oversampling(arg)
        # Adding generated LoRAS points from specific neighbourhood
        loras_set.extend(neighbourhood_loras_set)
...
def fit_resample(maj_class_points, min_class_points, k=None, num_shadow_points=None, list_sigma_f=None,
                 num_generated_points=None, num_hyp_points=1, num_aff_comb=None, random_state=42):
    ...
    if num_generated_points is None:
        import math
        num_generated_points = math.ceil(
            (len(maj_class_points) + len(min_class_points)) / (num_hyp_points * len(min_class_points)))
    ...
    return np.concatenate((min_class_points,
                           loras_oversampling(min_class_points_copy, k, num_shadow_points, list_sigma_f,
                                              num_generated_points, num_hyp_points, num_aff_comb, random_state)))
...
class LORAS:
    ...
    def fit_resample(self, x, y):
        ...
        ps = np.stack(ps)
        ns = np.stack(ns)
        ps = fit_resample(ns, ps, random_state=self.seed)
        x = np.concatenate([ps, ns], axis=0, dtype='float32')
        y = np.concatenate(
            [
                np.ones(len(ps)),
                np.zeros(len(ns)),
